...
- `5e93960` — docs: catat hash commit 9ed34ab & bump versi 2.66.5+144
  utk fix pre-order void + total cart bar
...
- `f45ba47` — fix(kasir): cegah race resume/tahan pesanan ditahan bikin data
  lenyap
- `8437c25` — fix(kasir): cegah double-tap tombol Tahan di cart sheet bikin
  held order duplikat
...
- chore: bump BUILD ke `2.48.0+102` setelah merge tiga sesi paralel —
  ketiganya kebetulan sama-sama menaikkan `schemaVersion` ke 41 &
  bump BUILD berbeda-beda sebelum digabung, BUILD dinaikkan sekali lagi
  supaya tidak ada rilis beda isi dgn nomor BUILD sama.
