...
_Update sesi 6 September 2026 (sesi ketiga puluh — DUA sesi paralel:
"Lunasi Hutang" dari keranjang aktif & "Batalkan & Susun Ulang"). Versi
kerja **2.48.0+101** (MINOR naik, PATCH reset — kedua perubahan terlihat
pengguna; masing-masing sesi kebetulan sama-sama bump ke `2.48.0+100`
sebelum digabung, BUILD dinaikkan sekali lagi jadi `+101` di commit
merge). schemaVersion **40 -> 41** (kolom baru `transactions.
debt_settlement_detail`, TEXT nullable, aditif murni — dari fitur "Lunasi
Hutang"; fitur "Batalkan & Susun Ulang" TIDAK menyentuh skema DB sama
sekali, semua state barunya murni Dart-side via `CartMeta.replacesTxId`
persisted SharedPreferences)._

## Sesi ini — "Lunasi Hutang" dari keranjang aktif SELESAI (`a921860`)
